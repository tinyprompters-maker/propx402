  ...
  return async (req, res, next) => {
    ...
    if (!xPayment) {
      return res.status(402).json({
        ...
        x402Version: 2,
        accepts: [{
          scheme: 'exact', price: `$${price}`, network: NETWORK_CAIP2, payTo: WALLET_ADDRESS,
...
app.get('/health', (req, res) => {
  res.json({
    ...
    network: 'base-mainnet', networkCAIP2: NETWORK_CAIP2,
    wallet: WALLET_ADDRESS, facilitator: FACILITATOR_URL,
    scheme: 'exact', token: 'USDC',
    ...
    endpoints: {
      ...
      '/property-narrative': { price: '$0.10 USDC',    method: 'POST', body: '{ address: string }',                     description: 'Gemini 2.0 Flash AI investment narrative and due diligence report' },
    ...
    freeEndpoints: [
      'GET /health', 'GET /.well-known/x402', 'GET /.well-known/mcp', 'GET /openapi.json',
      'POST /test/property-intel', 'POST /test/property-investor',
      'POST /test/property-narrative', 'POST /test/property-full',
    ],
    dataSources: [
...
// ─── FREE: x402 V2 Discovery ──────────────────────────────────────────────────
app.get('/.well-known/x402', (req, res) => {
  res.json({
    version: '2.0', name: 'PropX402',
    description: 'Real estate property intelligence API for AI agents — 14 data sources, 5 investor strategy engines, Gemini AI narratives',
    category: 'real-estate',
    tags: ['real-estate', 'property-intel', 'investor-analysis', 'ai-agents', 'mcp', 'proptech'],
    payTo: WALLET_ADDRESS, facilitator: FACILITATOR_URL,
    network: NETWORK_CAIP2, scheme: 'exact', token: 'USDC',
    paymentGateway: `https://api.paysponge.com/x402/purchase/${SPONGE_SERVICE_ID}`,
    endpoints: [
      ...
      { method: 'POST', path: '/property-narrative', price: '$0.10', description: 'Gemini 2.0 Flash AI investment narrative and due diligence report' },
...
app.get('/.well-known/mcp', (req, res) => {
  res.json({
    schema_version: '1.0', name: 'PropX402',
    ...
    tools: [
      ...
      },
      {
        ...
        description: 'Get Gemini 2.0 Flash AI-generated investment analysis and due diligence report',
...
app.get('/openapi.json', (req, res) => {
  res.json({
    ...
    paths: {
      ...
      '/property-narrative':      { post: { summary: 'Gemini 2.0 Flash AI investment narrative — $0.10 USDC',      security: [{ x402: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { address: { type: 'string' } }, required: ['address'] } } } }, responses: { '200': { description: 'AI narrative' }, '402': { description: 'Payment required' } } } },
...
// ─── PAID: AI Narrative ───────────────────────────────────────────────────────
app.post('/property-narrative', requirePayment('0.10', 'PropX402 — Gemini 2.0 Flash AI investment narrative'), async (req, res) => {
  const { address } = req.body;
  if (!address) return res.status(400).json({ error: 'address is required' });
  try {
    const intel     = await getPropertyIntel(address);
